...
Confirmed specific problematic dates.  
...
weekly_horizons <- c(1, 2, 3, 4)  # horizons in weeks for the weekly pairs
  ...
  ggplot(weekly_pair_counts, aes(x = rank_t, y = horizon, fill = n)) +
    geom_tile() +
    scale_x_log10(breaks = params$ranks_keep) +
    scale_fill_gradient(low = "white", high = "steelblue", name = "pairs") +
    labs(
      title = "Observed (t, t+H) weekly pair counts by rank and horizon",
      x = "Rank at week t (log scale)",
      y = "Horizon (weeks)"
    )


# Heatmap over all ranks
ggplot(pair_counts, aes(x = rank_t, y = factor(horizon), fill = n)) +
  geom_tile() +
  scale_x_log10(breaks = params$ranks_keep) +
  scale_fill_gradient(low = "white", high = "steelblue", name = "pairs") +
  labs(
    title = "Observed (t, t+H) pair counts by rank and horizon",
    x = "Rank at t (log scale)", y = "Horizon (days)"
  )
...
# Rank-conditioned growth overlays: raw and standardized

Objective. Overlay rank-conditioned densities for (g) and standardized (g/\sqrt{\log r}).

Rationale. Shows whether dispersion collapses under (\sqrt{\log r}) scaling. Useful to contrast log-rank variance vs Atlas.

Checks. Common x-limits across horizons. Rank 1 excluded from standardized view. Adequate sample counts.

```{r standardized growth}
build_growth_by_rank <- function(daily, horizons = c(1,7,28), ranks_keep = params$ranks_keep) {
  base <- daily %>% transmute(date, endpoint_id, rank_t = as.integer(rank), a_t = metric_value)
  out <- lapply(horizons, function(H) {
    y <- base %>% transmute(date = date - days(H), endpoint_id, a_tpH = a_t)
    base %>%
      inner_join(y, by = c("date","endpoint_id")) %>%
      filter(a_t > 0, a_tpH > 0) %>%
      mutate(horizon = H, g = log(a_tpH) - log(a_t)) %>%
      filter(rank_t %in% ranks_keep)
  }) %>% bind_rows()

  out_std <- out %>%
    mutate(scale = sqrt(log(pmax(rank_t, 1))),
           g_std = if_else(rank_t > 1 & is.finite(scale) & scale > 0, g/scale, NA_real_)) %>%
    filter(!is.na(g_std))

  list(raw = out, std = out_std)
}

rank_overlay_plots <- function(growth_df, value_col = c("g","g_std"), ranks_overlay = params$ranks_overlay,
                               horizons = params$horizons, min_n_per_rank = params$min_n_per_rank, line_width = 1.0) {
  value_col <- match.arg(value_col)
  df_all <- growth_df %>% filter(horizon %in% horizons, rank_t %in% ranks_overlay, is.finite(.data[[value_col]]))
  xlim_global <- range(df_all[[value_col]], na.rm = TRUE)

  lapply(horizons, function(H) {
    dH <- df_all %>% filter(horizon == H) %>%
      group_by(rank_t) %>% filter(n() >= min_n_per_rank) %>% ungroup()
    if (nrow(dH) == 0) return(ggplot() + theme_void() + ggtitle(sprintf("No data for H = %s", H)))
    bwH <- tryCatch(stats::bw.SJ(dH[[value_col]]), error = function(e) stats::bw.nrd0(dH[[value_col]]))
    ggplot(dH, aes(x = .data[[value_col]], colour = factor(rank_t), group = factor(rank_t))) +
      geom_density(bw = bwH, linewidth = line_width) +
      geom_vline(xintercept = 0, linetype = "dotted") +
      coord_cartesian(xlim = xlim_global) +
      labs(title = sprintf("Rank-conditioned growth (%s), H = %sd", ifelse(value_col=="g","raw","standardized"), H), subtitle = paste0("Overlaid ranks: ", paste(ranks_overlay, collapse = ", "), " | common x-scale"), x = "Logged growth rate", y = "Density", colour = "Rank at t")
  })
}

G <- build_growth_by_rank(daily, horizons = params$horizons, ranks_keep = params$ranks_keep)
purrr::walk(rank_overlay_plots(G$raw, "g"), print)
purrr::walk(rank_overlay_plots(G$std, "g_std"), print)
...
What we learned.
	•	CDC head exponent and fit quality summarize concentration.
	•	Variance funnel vs constant variance contrasts log-rank vs Atlas.
	•	Leakage rates by rank and horizon quantify selection pressure.
	•	Leakage-aware densities and Lee bounds isolate distribution shape from missingness.
	•	Weekly aggregation stabilizes growth diagnostics.

What would falsify the approach.
	•	CDC unstable across nearby windows.
	•	Drift positive for top ranks over long spans.
	•	Variance slopes flip sign under minor filters.
	•	IPW densities explode due to (\pi) near zero.
	•	Weekly diagnostics contradict daily in systematic ways.
