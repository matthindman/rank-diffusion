test_that("excess_kurtosis uses the bias-corrected Fisher definition", {
  ...
  expect_equal(rankdiff:::excess_kurtosis(x), expected)
})
...
test_that("numeric_gradient matches NumPy on a nonuniform grid", {
  ...
  expect_equal(rankdiff:::.numeric_gradient(y, x), expected, tolerance = 1e-8)
})
...
test_that("select_tracked_ids does not require an existing RNG state", {
  ...
  ids <- rankdiff:::.select_tracked_ids(data.frame(), letters[1:5], 2L, 42L)
...
test_that("save_fit_result and load_fit_result preserve config and param classes", {
  ...
  threshold <- rankdiff:::new_threshold_model(c(1, 2), c(1, 2))
  ...
  path <- file.path(tempdir(), paste0("rankdiff-test-", Sys.getpid()))
