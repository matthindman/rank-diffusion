...
head_weeks <- weekly %>% 
  ...
  filter(n_ranks >= K_head, min_r == 1L, max_r >= K_head)
...
glue("Using {n_weeks} weeks with ranks 1:{K_head}.") %>% print()
```
...
# Empirical CDC head m̂(k)
mhat <- weekly_head %>%
  group_by(k = rank_w) %>%
  summarise(m_hat = mean(log_share), .groups = "drop") %>%
  arrange(k)

# Observed top-1000 share by week
...
```{r const-variance}
decile_cut <- cut(1:K_head, breaks = quantile(1:K_head, probs = seq(0,1,0.1)), include.lowest = TRUE, labels = FALSE)
decile_df <- tibble(k = 1:K_head, decile = decile_cut)
...
## 4.2 Rank‑drift schedule \\(g_k\\) on the head

```{r drift-estimation}
# Prepare observed growth series for isotonic fit
rs_fin <- rank_stats %>% filter(is.finite(bar_g), is.finite(n_k), n_k > 0) %>% arrange(k)
k_obs <- rs_fin$k
y_obs <- rs_fin$bar_g
w_obs <- pmax(1, rs_fin$n_k)
K_fit <- length(y_obs)

lambda_tv <- params$lambda_tv

# Gap-based full head drift as a fallback and to fill missing ks
# Δ̄_k exists for k = 1..K_head-1 ideally
# Build r_gap and g_hat_gap_full of length K_head
Delta_vec <- Delta_bar$overline_Delta_k
if (length(Delta_vec) < 1) stop("No adjacent gap information available.")
# Ensure vector aligned to k=1..K_head-1
Delta_full <- tibble(k = 1:(K_head-1)) %>% left_join(Delta_bar, by="k") %>%
  mutate(overline_Delta_k = ifelse(is.finite(overline_Delta_k), overline_Delta_k, median(Delta_bar$overline_Delta_k, na.rm = TRUE))) %>%
  pull(overline_Delta_k)

r_gap <- 1 / Delta_full
G_gap <- -(sigma2_hat/2) * r_gap
g_gap_seq <- c(G_gap[1], diff(G_gap))
g_hat_gap_full <- c(g_gap_seq, tail(g_gap_seq,1))  # length K_head

# Try CVXR isotonic with TV penalty if at least 2 observations
g_hat_obs <- NULL; gamma_hat <- 0
if (K_fit >= 2) {
  g <- CVXR::Variable(K_fit)
  gamma <- CVXR::Variable(1)
  resid <- y_obs - gamma - g
  loss <- sum(w_obs * CVXR::square(resid))
  tv <- if (K_fit > 1) sum(CVXR::abs(g[2:K_fit] - g[1:(K_fit-1)])) else 0
  constraints <- list(g[1:(K_fit-1)] <= g[2:K_fit])
  prob <- CVXR::Problem(CVXR::Minimize(loss + lambda_tv * tv), constraints)
  res <- tryCatch(CVXR::solve(prob, solver = "OSQP"), error = function(e) NULL)
  if (!is.null(res) && !is.null(res$value)) {
    g_hat_obs <- as.numeric(value(g)); gamma_hat <- as.numeric(value(gamma))
  }
} else if (K_fit == 1) {
  # Single point: center to zero
  g_hat_obs <- y_obs - weighted.mean(y_obs, w_obs); gamma_hat <- weighted.mean(y_obs, w_obs)
}

# Build a full head drift vector using gap-based values as baseline, then overwrite observed ks
g_hat_full <- g_hat_gap_full
if (!is.null(g_hat_obs)) {
  g_hat_full[k_obs] <- g_hat_obs
}

# Monotone adjust over the full head with weights: use observed counts where available else 1
w_full <- rep(1, K_head)
w_full[k_obs] <- w_obs
g_hat_full <- isotone::gpava(1:K_head, g_hat_full, weights = w_full, decreasing = FALSE)$x %>% as.numeric()

drift_hat <- tibble(k = 1:K_head, g_hat = g_hat_full, gamma_hat = gamma_hat)

# For comparison table
g_hat_gap <- g_hat_gap_full
drift_compare <- tibble(k = 1:K_head, g_hat = g_hat_full, g_hat_gap = g_hat_gap)
write_csv(drift_compare, "outputs/tables/drift_compare_head.csv")
```

## 4.3 Tail extrapolation for \\(k>K_{head}\\)

```{r tail-fit}
k0_candidates <- seq(params$k0_min, params$k0_max, by = 50)
tail_family <- c("power","log")

# Use occupancy weights, defaulting to 1 for missing ranks
w_full <- tibble(k = 1:K_head) %>% left_join(rank_stats %>% dplyr::select(k, n_k), by="k") %>%
  transmute(w = ifelse(is.na(n_k) | n_k <= 0, 1, n_k)) %>% pull(w)
...
tail_choice <- choose_tail(k0_candidates, tail_family, 1:K_head, drift_hat$g_hat, w_full)
...
tail_predict <- function(kk){
  ...
  cummax(y) # monotone non-decreasing toward zero
}
```
...
score_N <- function(N){
  ...
  S_mod_1000 <- sum(MCE[1:K_head])
  m_model_head <- cdc$m[1:K_head]
  rmse_head <- sqrt(mean((m_model_head - mhat$m_hat[1:K_head])^2, na.rm = TRUE))
...
g_tail_only <- tail_predict((K_head+1):best$N)
tail_df <- tibble(k = (K_head+1):best$N, g_tail = g_tail_only)
...
core_pipeline_from_weeks <- function(weeks_subset){
  ...
  rs <- og %>% group_by(k) %>% summarise(bar_g = mean(r_f), n_k = n(), .groups = "drop") %>% arrange(k)
  wy_mean <- stats::weighted.mean(rs$bar_g, rs$n_k)
  g_iso <- isotone::gpava(rs$k, rs$bar_g - wy_mean, weights = rs$n_k, decreasing = FALSE)$x %>% as.numeric()
  v_k <- og %>% group_by(k) %>% summarise(v_k = var(r_f), n_k = n(), .groups="drop")
  sig2 <- stats::weighted.mean(v_k$v_k, v_k$n_k)
  G <- cumsum(g_iso)
  ...
  list(g = g_iso, r = r, m = m, top1000_share_mod = sum(MCE[1:min(K_head, length(MCE))]))
}
...
cdc_df_tail <- tibble(k = (K_head+1):best$N, m_hat = m_final[(K_head+1):best$N], part = "estimated")
...
checks <- list(
  ...
  top1000_share_close = abs(sum(MCE[1:K_head]) - S_obs_1000) < 0.05
)
checks
