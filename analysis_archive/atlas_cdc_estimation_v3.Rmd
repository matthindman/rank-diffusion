...
config <- modifyList(config, list(
  ...
  use_piecewise_sigma = TRUE        # prefer piecewise sigma² if built
))
```
...
# NEW: build piecewise sigma²(k): top (1..L)=smoothed, body (L..U]=scalar or light-smooth, tail (>U)=smoothed
...
# Build piecewise sigma² table over observed ranks 1:k_full
...
# Plot overlay: raw v_k with piecewise sigma²(k)
...
2) For each R element of  {4,13,26,52} and K element of  {1000,2000,5000}, slide R‑week windows
3) For each window start t: fix L_t (top‑K at t), sum captured mass over s element of  [t, t+R−1], and total mass over those weeks
