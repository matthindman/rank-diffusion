...
suppressPackageStartupMessages({
  ...
  library(slider); library(glue); library(arrow)
})
...
# If daily/weekly already exist, use them; otherwise read Parquet
if (!(exists("daily", inherits = TRUE) && exists("weekly", inherits = TRUE))) {
  ...
  # If total_metric missing or mostly NA, compute per-date totals from metric_value
  if (!("total_metric" %in% names(daily)) || mean(is.na(daily$total_metric)) > 0.25) {
    ...
    # Use distinct platform totals per day if provided
    totals_by_day <- daily %>%
      group_by(date) %>%
    ...
    # If still NA for some days, backfill from sums
  ...
  # Only use the _day suffix column if it exists, otherwise the join already updated total_metric
  if ("total_metric_day" %in% names(daily)) {
    daily <- daily %>% mutate(total_metric = coalesce(total_metric, total_metric_day)) %>% select(-total_metric_day)
  }
  ...
  # Compute rank if missing: deterministic tie-break by endpoint_id
  ...
  # Build weekly totals from distinct daily totals
  weekly_totals <- daily %>%
  ...
  # Build weekly per-endpoint metrics
...
# Next-week log_share per endpoint
...
# Consecutive weeks only and presence at t and t+1
...
g_gap <- c(G_gap[1], diff(G_gap)) # length K-1 => g_1..g_(K-1); extend to K
...
fit_tail_family <- function(k0, fam, k, gk, w){
  ...
  if (fam == "power") {
    ...
    start <- list(a = max(1e-8, -df$y[df$k==k0][1] %||% abs(median(df$y))), alpha = 0.5)
    mod <- tryCatch(
    ...
    start <- list(a = max(1e-8, -df$y[df$k==k0][1] %||% abs(median(df$y))), c = 1.0)
    mod <- tryCatch(
...
enforce_stationarity <- function(g, N, m_seq = c(50,100,200,500,1000,2500,5000)){
  S <- sum(g)
  if (S >= 0) return(NULL) # pathological; skip this N
