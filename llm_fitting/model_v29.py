#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v2.9
===============================================
Fixes from v2.8:

v2.8 failures: perm_boost=2.12 required κ=0.020 (HL=35wk) for stationarity,
which destroyed long-horizon persistence (Pers(13)=29 vs 64), collapsed VR
(only 1/4 pass), and worsened RACF (errors 0.22-0.33).

Root cause: c initialization (τ₀ = obs - c₀) inflated Var(τ₀) and the
analytical R² gap formula overestimated the required perm_boost.

v2.9 changes:
1. BURN-IN instead of c initialization: Run 50 weeks before recording to let
   the transitory component reach its stationary distribution naturally. This
   avoids the initialization decomposition problem entirely and gives the
   simulation proper R² behavior from the first recorded week.

2. RANK-DEPENDENT mean-reversion: Pull τ toward the Zipf target μ(r) at the
   endpoint's current rank, NOT toward the global mean. This preserves the
   Zipf shape while stabilizing cross-sectional variance.

3. perm_boost=1.0 (removed): The burn-in naturally provides the R² correction
   that v2.8 tried to achieve with perm_boost. Without burn-in, c=0 at t=0
   made R²(1) artificially high. With 50-week burn-in, c is at stationary and
   R²(1) should be naturally lower.

4. κ at base value (0.004, HL~160wk): Not scaled by perm_boost².

Keeps from v2.8: within-endpoint t_df estimation (→ kurtosis fixed).
"""
...
print(f"  κ={kappa:.6f} (rank-dep mean rev, half-life={np.log(2)/kappa:.0f}wk)")
...
# Zipf target: the sorted initial observations define μ(rank)
# For rank-dependent mean-reversion: pull τ_i toward zipf_target[rank_i - 1]
zipf_target = w0_sorted.copy()  # μ(r) for r = 1, 2, ..., N_FULL
...
for t_abs in range(1, T_TOTAL):
    ...
    # Rank-dependent mean reversion: pull toward Zipf target at current rank
    # τ_i += η_i - κ × (τ_i - μ(rank_i))
    tau += eta - kappa * (tau - zipf_target[cr - 1])
...
fig.suptitle(f'Rank Diffusion v2.9 | {n_pass}/{len(tests)} | σ_obs={sigma_obs:.3f} κ={kappa:.5f} t_df={t_df:.1f} (all estimated)',
...
plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v29_diagnostics.png', dpi=130, bbox_inches='tight')
print("Saved v29_diagnostics.png")
print("Done.")
