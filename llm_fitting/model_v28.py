#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v2.8
===============================================
Three targeted fixes from v2.7:

1. t_df: Now estimated from WITHIN-ENDPOINT standardized residuals (MLE).
   v2.7 used aggregate changes, double-counting σ_het heterogeneity → t_df=3.0
   → kurtosis=48.8 (catastrophic). Within-endpoint residuals remove
   between-endpoint variance heterogeneity, giving a proper innovation t_df.

2. Initialize c from stationary distribution (not zero).
   v2.7 set c_i,0=0, meaning no transitory variance at t=0. The empirical
   t=0 data already contains transitory components, so y=τ+c+ε. Setting
   τ_i,0 = obs_i - c_i,0 properly decomposes initial values. This fixes
   R² being too high (the simulation was comparing a "pure τ" t=0 with
   a "τ+c" t=k, inflating apparent persistence).

3. perm_boost estimated from R² gap.
   R²(1) depends on the ratio of permanent innovations to total cross-sec
   variance. We solve for the required σ²_η from the observed R² decay
   and compute perm_boost = sqrt(required_σ²_η / estimated_σ²_η).

All other parameters remain principled (σ_obs, σ_het, κ from v2.7).
"""
...
# ============================================================
print("\n" + "=" * 70)
print("STAGE 7: ESTIMATE perm_boost FROM R² GAP")
print("=" * 70)

# R²(k) ≈ Var(τ_0)² / [(Var(τ_0) + V_rest_0) × (Var(τ_0) + k×σ²_η_eff + V_rest_0)]
# where V_rest_0 = Var(c_0) + σ²_obs, and σ²_η_eff = E[h²]×mean(σ²_η)×perm_boost²
#
# The R² DECAY from lag 0 to lag k depends on σ²_η_eff:
#   R²(k) ≈ Var_τ / (Var_τ + k × σ²_η_eff)  [simplified, ignoring transitory effects]
#
# From empirical data:
#   R²(1) = 0.79, R²(13) = 0.67
#   The decay rate gives us the required σ²_η_eff
#
# Inversion: from R²(k) = V / (V + k×σ²_η_eff)
#   σ²_η_eff = V × (1/R²(k) - 1) / k
#
# Use R²(1) as the primary target (most precise, least affected by transitory dynamics)

Var_tau_0 = xsec_var_full  # Cross-sectional variance of log-activity at t=0

# Estimate V_rest = Var(c_0) + σ²_obs from the band parameters
# For stationary c: Var(c) = sum over bands of n_b/N × σ²_c(r_b) × E[h²]
# where σ²_c = σ²_ν / (1 - φ²)
mean_sc2 = sum(band_params[(lo,hi)]['sn2'] / (1 - band_params[(lo,hi)]['phi']**2) *
               band_stats[(lo,hi)]['n'] for lo,hi in band_params) / total_n
V_c_stationary = E_h2 * mean_sc2
V_rest = V_c_stationary + sobs2

# Effective permanent variance from R²(1) target
# R²(1) ≈ (Var_tau_0 - σ²_η_eff)² / [(Var_tau_0 + V_rest - σ²_η_eff) × (Var_tau_0 + V_rest)]
# Simplified (σ²_η_eff << Var_tau_0):
# R²(1) ≈ Var_tau_0 / (Var_tau_0 + σ²_η_eff + V_rest × something)
# Better: use the exact formula for R² of a RW + noise:
# R²(k) = Var(τ_k) / [Var(τ_k) + V_rest]
# where Var(τ_k) = Var(τ_0) - 2k×κ×Var(τ_0) + k×σ²_η_eff (approx for small κ, k)
# Actually, more precisely:
# Cov(y_0, y_k) = Var(τ) × (1 - κ)^k + Cov(c_0, c_k) + [obs noise term = 0 for k≥1]
# Var(y_0) = Var(τ) + Var(c) + σ²_obs
# Var(y_k) ≈ Var(τ) + Var(c) + σ²_obs (stationarity)
# So R²(k) = [Var(τ)×(1-κ)^k + Var(c)×φ^k]² / [Var(τ) + Var(c) + σ²_obs]²
#
# But we need the R² to reflect BOTH the persistent correlation AND the displacement
# from new permanent shocks. The key insight: R²(1) measures how well t=0 values
# predict t=1 values. The displacement comes from permanent innovations.
#
# The simplest principled approach: solve for perm_boost such that the simulated
# R²(1) matches the empirical R²(1). Use the analytical formula:
#
# Corr(y_0, y_1)² = [Var(τ)(1-κ) + V_c_stat × φ_eff]² / [Var(τ) + V_c_stat + σ²_obs]²
#
# where the displacement from permanent innovations reduces the correlation:
# At lag 1: total new variance ≈ σ²_η_eff (added to denominator, not correlated with t=0)
#
# More carefully:
# Var(y_1) ≈ Var(τ) + σ²_η_eff + V_c_stat + σ²_obs
# Cov(y_0, y_1) = Var(τ)(1-κ) + V_c_stat × φ_eff
# R²(1) = Cov²/(Var(y_0)×Var(y_1))

# Compute population-weighted average φ for the aggregate transitory correlation
mean_phi = sum(band_params[(lo,hi)]['phi'] * band_stats[(lo,hi)]['n']
               for lo,hi in band_params) / total_n

Var_tau = Var_tau_0 - V_c_stationary - sobs2  # Pure permanent cross-sec variance
Var_tau = max(Var_tau, Var_tau_0 * 0.5)  # Ensure reasonable

# Current σ²_η_eff with perm_boost=1.0
sigma2_eta_eff_base = E_h2 * mean_se2 * jump_var_factor

# Target: solve for perm_boost
# R²(1) = [Var_τ(1-κ) + V_c_stat × mean_φ]² / [(Var_τ + V_c_stat + σ²_obs) × (Var_τ + pb² × σ²_η_eff_base + V_c_stat + σ²_obs)]
R2_target = xr2_emp[1]
cov_term = Var_tau * (1 - kappa) + V_c_stationary * mean_phi
var_y0 = Var_tau + V_c_stationary + sobs2

# R²(1) = cov²/(var_y0 × (var_y0 + pb²×σ²_η_eff_base))
# var_y0 × (var_y0 + pb²×σ²_η_eff_base) = cov²/R²_target
# var_y0 + pb²×σ²_η_eff_base = cov²/(R²_target × var_y0)
# pb² = [cov²/(R²_target × var_y0) - var_y0] / σ²_η_eff_base
pb2_required = (cov_term**2 / (R2_target * var_y0) - var_y0) / sigma2_eta_eff_base
perm_boost = np.sqrt(max(1.0, pb2_required))  # Floor at 1.0 (never reduce)
perm_boost = min(perm_boost, 5.0)  # Cap to prevent instability

print(f"  Var(τ) = {Var_tau:.4f}, V_c_stat = {V_c_stationary:.4f}, σ²_obs = {sobs2:.4f}")
print(f"  Var(y_0) = {var_y0:.4f}")
print(f"  Cov(y_0,y_1) = {cov_term:.4f}, mean_φ = {mean_phi:.4f}")
print(f"  σ²_η_eff (base) = {sigma2_eta_eff_base:.6f}")
print(f"  R²(1) target = {R2_target:.4f}")
print(f"  pb² required = {pb2_required:.4f}")
print(f"  perm_boost = {perm_boost:.4f}")

# Verify: predicted R²(1) with this perm_boost
var_y1 = var_y0 + perm_boost**2 * sigma2_eta_eff_base
r2_predicted = cov_term**2 / (var_y0 * var_y1)
print(f"  Predicted R²(1) = {r2_predicted:.4f} (target = {R2_target:.4f})")

# Re-derive κ with perm_boost
sigma2_eta_eff = sigma2_eta_eff_base * perm_boost**2
kappa_new = sigma2_eta_eff / (2 * xsec_var_full)
kappa_new = max(kappa_new, 0.0005)
print(f"\n  Updated κ = {kappa_new:.6f} (was {kappa:.6f})")
print(f"  Updated half-life = {np.log(2)/kappa_new:.0f} weeks (was {np.log(2)/kappa:.0f})")
kappa = kappa_new
...
print(f"  perm_boost= {perm_boost:.4f} (from R² gap)")
...
print(f"  perm_boost={perm_boost:.4f}, jump_prob={jump_prob:.4f}, jump_scale={jump_scale:.2f}")
...
# Initialize c from stationary distribution (NOT zero)
# v2.7 set c=0, meaning no transitory variance at t=0. But the empirical
# t=0 data already contains transitory components: y = τ + c + ε.
# So we draw c_i,0 ~ N(0, σ²_c(r_i) × h²_i) and set τ_i,0 = obs_i - c_i,0.

np.random.seed(42)  # Set seed before het_multiplier generation
het_multiplier = np.exp(np.random.normal(0, sigma_het, N_FULL))
het_multiplier = np.clip(het_multiplier, 0.15, 8.0)

# Get rank-dependent parameters for initial ranks
init_ranks = np.arange(1, N_FULL + 1)
se_init, phi_init, sn_init = get_p(init_ranks)

# Stationary variance of c: σ²_c = σ²_ν / (1 - φ²)
sn2_init = sn_init**2
sc2_init = sn2_init / (1 - phi_init**2)

# Draw c from stationary distribution, scaled by heterogeneity
c = np.random.normal(0, 1, N_FULL) * np.sqrt(sc2_init) * het_multiplier

# Decompose: τ = observed - c  (ignoring obs noise in initialization,
# since we don't know the actual obs noise realization)
tau = w0_sorted.copy() - c
tau_global_mean = np.mean(tau)

print(f"  Init c: mean={np.mean(c):.4f}, std={np.std(c):.4f}, var={np.var(c):.4f}")
print(f"  Init τ: var={np.var(tau):.4f}")
print(f"  Init y=τ+c: var={np.var(tau+c):.4f} (emp={xsec_var_full:.4f})")
...
fig.suptitle(f'Rank Diffusion v2.8 | {n_pass}/{len(tests)} | σ_obs={sigma_obs:.3f} κ={kappa:.5f} t_df={t_df:.1f} (all estimated)',
...
plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v28_diagnostics.png', dpi=130, bbox_inches='tight')
print("Saved v28_diagnostics.png")
print("Done.")
