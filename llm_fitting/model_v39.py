#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v3.9 — Cross-Sectional Variance Stabilization
================================================================================
Changes from v3.8 (15/15, two-pass kurtosis calibration):

v3.8's diagnostic plots revealed that the cross-sectional τ variance drifts upward
~40% over the 88-week recording period (3.23 → 4.50), while the empirical cross-
sectional variance is stable (~3.18). This is visible in both the cross-sec variance
time series and the CDC (Capital Distribution Curve) plots, and would undermine the
paper's implicit stationarity assumption.

Root cause: The analytical κ formula (κ_base = E[η²] / (2·E[rank_weight·dev²]))
assumes a homogeneous linear model, but underestimates the required mean-reversion
because it doesn't account for:
  1. Heterogeneity amplification (het_multiplier increases effective η variance)
  2. The transitory component's contribution to level variance
  3. ARCH effects on effective innovation variance
  4. Rank-reassignment dynamics that change the effective weighted κ over time

Key change:
  κ variance-stabilization factor (1.20×): Multiply κ_base by 1.20 to bring
  the model's stationary τ variance closer to the initial condition, reducing
  the recording-period drift from ~1.27 to ~0.93. Larger factors (1.25×, 1.50×)
  reduce drift more but push Pers(13) past its threshold.

Why this also helps calibration diagnostics:
  - R²: sim R² is consistently ABOVE emp → stronger κ reduces persistence → closer
  - RACF: sim RACF is above emp → same mechanism → closer
  - Pers(13): diff=-8.4 (threshold 10, margin 1.6) → κ(top ranks) is tiny,
    so 13-week top-100 persistence barely affected

All v3.8 features retained: two-pass kurtosis calibration, ARCH(1), rank-dep t_df.
"""
...
print("\n" + "=" * 70)
print("PARAMETER SUMMARY (v3.9: v3.8 + variance stabilization)")
print("=" * 70)
print(f"  σ_obs     = {sigma_obs:.4f}")
...
print(f"\n{'='*70}")
print(f"SIMULATION v3.9 — {N_REP} MC REPS — ARCH(1) + calibrated rank-dep t_df")
print(f"{'='*70}")
...
print(f"\n{'='*70}")
print("KURTOSIS CALIBRATION PASS (v3.9)")
print(f"{'='*70}")
...
print(f"\n{'='*70}")
print(f"SUMMARY v3.9 ({N_REP} replications)")
print(f"{'='*70}")
...
# ============================================================
print(f"\n{'='*70}")
print("PUBLICATION DIAGNOSTICS (v3.9)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v39_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v39_diagnostics.png")
    plt.close()
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v39_pub_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v39_pub_diagnostics.png")
    plt.close()

# Final summary
print(f"\n{'='*70}")
print(f"v3.9 COMPLETE")
print(f"{'='*70}")
print(f"  Calibration: {n_pass}/{len(tests)} diagnostics pass")
print(f"  Parameters: σ_obs={sigma_obs:.4f} σ_het={sigma_het:.4f}")
print(f"              t_df (calibrated): {', '.join(f'{lo}-{hi}={band_tdf[(lo,hi)]:.1f}' for lo,hi in bands)}")
print(f"              κ_base={kappa_base:.6f} (raw={kappa_base_raw:.6f} × stab={kappa_stab_factor:.2f})")
print(f"              T_burnin={T_BURNIN}")
print(f"              jump_p={jump_prob:.4f} jump_s={jump_scale:.2f}")
print(f"              α_arch={alpha_arch:.4f}")
print(f"  Key change: Cross-sec variance stabilization (κ×{kappa_stab_factor}, burnin={T_BURNIN})")
