#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v4.2 — Audit Fixes + Refactored Engine
================================================================================
Changes from v4.0 (v3.9 core + ablation + sensitivity):

v4.2 applies 14 audit-identified fixes with no changes to the model's statistical
structure or estimated parameters. The core simulation is identical to v3.9.

Bug Fixes:
  1. Persistence computed on endpoint IDs, not array positions.
  2. ARCH comment corrected: clipping means E[arch_var] > 1 (small bias ~0.5%).
  3. Per-band t_df correction comment rewritten to match actual intent.
  4. phi_agg guarded against small denominator.
  5. Optimizer: bare except -> except Exception; guard best is None.

Refactoring:
  6. Unified run_sim() replaces 4 duplicated sim functions (~280 lines vs ~620).
  7. Removed dead calibration loop (5 wasted run_simulation reps).
  8. Pathlib-relative paths replace all hardcoded absolute paths.
  9. Sort on log_y (monotonic, avoids redundant exp).
  10. Publication diagnostics use seeded RandomState(12345).
  11. Ablation/sensitivity harmonized to N_REP=25 (was 15/10).
  12. Removed perm_boost (always 1.0, a no-op).

Enhancements:
  13. MC uncertainty reporting: SE, distance-to-threshold in SE units, fragility.
  14. Kurtosis calibration comments clarify heuristic nature.
"""
...
# ============================================================
# DATA LOADING (same as v3.4, pathlib paths)
# ============================================================
print("=" * 70)
print("LOADING DATA")
print("=" * 70)

df = pd.read_parquet(DATA_DIR / 'fb_ranked_weekly_cutdown.parquet')
...
# Fix #4: Guard phi_agg against small denominator
if abs(acf_emp[2]) > 0.001:
    phi_agg = acf_emp[3] / acf_emp[2]
else:
    phi_agg = 0.5
    print(f"  WARNING: acf_emp[2]={acf_emp[2]:.6f} near zero; using phi_agg=0.5 fallback")
gamma1 = acf_emp[1] * emp_median_var
gamma2 = acf_emp[2] * emp_median_var

sigma2_obs_est = -gamma1 + gamma2 / phi_agg
clip_lo, clip_hi = 0.01**2, 0.50**2
sigma2_obs_clipped = np.clip(sigma2_obs_est, clip_lo, clip_hi)
sigma_obs = np.sqrt(sigma2_obs_clipped)
sobs2 = sigma_obs ** 2
print(f"  σ²_obs_est = {sigma2_obs_est:.6f} (unclipped)")
if sigma2_obs_est < clip_lo:
    print(f"  CLIPPING: σ²_obs_est below lower bound {clip_lo:.4f}; using σ_obs = {sigma_obs:.4f}")
elif sigma2_obs_est > clip_hi:
    print(f"  CLIPPING: σ²_obs_est above upper bound {clip_hi:.4f}; using σ_obs = {sigma_obs:.4f}")
else:
    print(f"  σ_obs = {sigma_obs:.4f} (no clipping)")
# Sensitivity to phi_agg: show how sigma_obs varies under alternative values
print(f"  phi_agg sensitivity (sigma_obs under alternative phi_agg):")
for phi_alt in [0.3, 0.5, 0.7]:
    s2_alt = -gamma1 + gamma2 / phi_alt
    s_alt = np.sqrt(np.clip(s2_alt, clip_lo, clip_hi))
    tag = " <-- used" if abs(phi_alt - phi_agg) < 0.001 else ""
    print(f"    phi_agg={phi_alt:.1f}: σ²_obs={s2_alt:.6f}, σ_obs={s_alt:.4f}{tag}")
...
# Fix #3: Per-band t_df estimation with observation-noise correction.
# The MLE on within-endpoint standardized residuals mixes signal and
# observation noise. When obs noise dominates (low signal_frac), residuals
# appear lighter-tailed than the true innovations, inflating the MLE df.
# Correction: for noise-dominated bands, scale df by 1/signal_frac to recover
# the latent tail weight. This is a heuristic bias correction, not a
# principled estimator — it closes the gap but is not exact.
...
for lo, hi in bands:
    ...
    if signal_frac < 0.30:
        ...
        df_corrected = min(df_corrected, 200.0)
    else:
...
# Fix #5: except Exception (not bare except); guard best is None
def fit_params(emp_var, emp_vr4, emp_acf1, emp_vr13=None, sobs2=0):
    ...
    for _ in range(200):
        ...
        try:
            ...
            pass
    if best is None:
        raise RuntimeError("Optimizer failed: no valid solution in 200 restarts")
...
# ============================================================
# Global κ for uniform (non-rank-dependent) mode; perm_boost removed (was 1.0)
kappa_global = kappa_base * np.mean((np.arange(1, N_FULL+1) / N_FULL) ** alpha_kappa)

print("\n" + "=" * 70)
print("PARAMETER SUMMARY (v4.2: v3.9 core + audit fixes)")
print("=" * 70)
...
print(f"  κ_global  = {kappa_global:.6f} (uniform-mode mean)")
...
# ============================================================
# UNIFIED SIMULATION KERNEL (v4.2: replaces 4 functions from v4.0)
# ============================================================
T_SIM = n_weeks
T_BURNIN = 50
...
print(f"\n{'='*70}")
print(f"SIMULATION v4.2 — {N_REP} MC REPS — unified kernel")
print(f"{'='*70}")
print(f"  N={N_FULL}, T_record={T_SIM}, T_burnin={T_BURNIN}, T_total={T_TOTAL}")


def run_sim(seed, *, config=None, return_extra=False, return_band_kurtosis=False):
    """
    Unified simulation kernel.

    config keys (all optional):
      Feature flags (default True): burn_in, kappa, rank_dep_kappa, kappa_stab,
          heavy_tails, arch, rank_dep_tdf, calibrated_tdf
      Parameter overrides: sigma_obs, sigma_het, kappa_base, alpha_kappa,
          alpha_arch, t_df_global

    Returns:
      return_band_kurtosis=True  -> dict{(lo,hi): kurtosis}
      return_extra=True          -> (diagnostics_dict, extra_arrays_dict)
      default                    -> (diagnostics_dict, None)
    """
    if config is None:
        config = {}
    rng = np.random.RandomState(seed)

    # Feature flags (default: all enabled = full v3.9)
    use_burnin = config.get('burn_in', True)
    use_kappa = config.get('kappa', True)
    use_rank_dep_kappa = config.get('rank_dep_kappa', True)
    use_kappa_stab = config.get('kappa_stab', True)
    use_heavy_tails = config.get('heavy_tails', True)
    use_arch = config.get('arch', True)
    use_rank_dep_tdf = config.get('rank_dep_tdf', True)
    use_calibrated_tdf = config.get('calibrated_tdf', True)

    # Parameter overrides
    s_obs = config.get('sigma_obs', sigma_obs)
    s_het = config.get('sigma_het', sigma_het)
    a_arch = config.get('alpha_arch', alpha_arch)
    a_kap = config.get('alpha_kappa', alpha_kappa)
    tdf_g = config.get('t_df_global', t_df_global)

    # kappa_base: explicit override > stab/unstab selection > zero
    if 'kappa_base' in config:
        kb = config['kappa_base']
    elif use_kappa:
        kb = kappa_base if use_kappa_stab else kappa_base_raw
    else:
        kb = 0.0
    if not use_kappa:
        kb = 0.0

    # Uniform κ for non-rank-dependent mode: mean of rank-dep schedule using local kb/a_kap.
    # This correctly reflects kappa_base_raw (not stabilized) when kappa_stab=False.
    kappa_uniform = kb * np.mean((np.arange(1, N_FULL+1) / N_FULL) ** a_kap) if kb > 0 else 0.0

    # t_df scaling for sensitivity perturbations
    tdf_scale = tdf_g / t_df_global if tdf_g != t_df_global else 1.0

    def get_tdf_local(ranks):
        lr = np.log(np.clip(ranks.astype(float), 1, bc_arr[-1]*2))
        arr = tdf_arr if use_calibrated_tdf else tdf_arr_precal
        base = np.interp(lr, np.log(bc_arr), arr)
        if tdf_scale != 1.0:
            return np.maximum(3.5, base * tdf_scale)
        return base
    ...
    het_multiplier = np.clip(np.exp(rng.normal(0, s_het, N_FULL)), 0.15, 8.0)
    ...
    sim_ly_true = np.zeros((T_SIM, N_FULL)) if return_extra else None

    # Initial ranking (Fix #9: sort on log_y, not exp — same result, avoids exp)
    obs_noise = rng.normal(0, s_obs, N_FULL)
    y0_obs = tau + c_state + obs_noise
    order = np.argsort(-y0_obs)
    ...
    if t_burnin == 0:
        sim_ly[0] = y0_obs; sim_rk[0] = ranks; sim_ids[0] = endpoint_id.copy()
        if return_extra:
            sim_ly_true[0] = tau + c_state
    ...
    for t_abs in range(1, t_total):
        ...
        # Fix #2: ARCH(1) scaling. Clipping z^2 at 4.0 truncates the upper tail,
        # so E[z^2_clipped] < 1, giving E[arch_var] = (1-a)+a*E[z^2] slightly
        # below 1. This means unconditional variance is slightly attenuated.
        if use_arch:
            arch_var = (1 - a_arch) + a_arch * last_z_sq
            arch_scale = np.sqrt(np.clip(arch_var, 0.1, 10.0))
        else:
            arch_scale = np.ones(N_FULL)
        ...
        if use_heavy_tails:
            if use_rank_dep_tdf:
                ...
                t_raw = sp_stats.t.rvs(df=tdf_g, size=N_FULL, random_state=rng)
                t_var_factor = np.sqrt(max(tdf_g - 2, 0.5) / tdf_g)
            nu = sn_het * t_var_factor * arch_scale * t_raw
        else:
            nu = sn_het * arch_scale * rng.normal(0, 1, N_FULL)
        ...
        # Cross-sectional variance measured pre-entry/exit by design:
        # captures diffusion dynamics, not compositional changes from turnover.
        ...
        if t_rec >= 0:
            ...
            if n_ex > 0:
                ...
                tau[exi] = new_tau
                # Entrant c_state: t-distributed when heavy_tails enabled
                if use_heavy_tails:
                    c_state[exi] = sp_stats.t.rvs(df=tdf_g, size=n_ex, random_state=rng) * 0.3
                else:
                    c_state[exi] = rng.normal(0, 0.3, n_ex)
                het_multiplier[exi] = np.clip(np.exp(rng.normal(0, s_het, n_ex)), 0.15, 8.0)
                last_z_sq[exi] = 1.0
                ep_type[exi] = 1
    ...
    # --- Band kurtosis mode: quick return ---
    if return_band_kurtosis:
        init_ids_set = set(sim_ids[0])
        survivors = init_ids_set.copy()
        for t in range(1, T_SIM):
            survivors &= set(sim_ids[t])
        survivor_list = sorted(survivors)
        nbp = len(survivor_list)
        bp_ly_bk = np.zeros((T_SIM, nbp))
        bp_rk_bk = np.zeros((T_SIM, nbp), dtype=int)
        for t in range(T_SIM):
            id_map = {eid: idx for idx, eid in enumerate(sim_ids[t])}
            for j, sid in enumerate(survivor_list):
                bp_ly_bk[t, j] = sim_ly[t, id_map[sid]]
                bp_rk_bk[t, j] = sim_rk[t, id_map[sid]]
        bp_avg_rk_bk = pd.DataFrame(bp_rk_bk).mean()
        sim_ch_bk = pd.DataFrame(bp_ly_bk).diff().iloc[1:]
        bk = {}
        for lo, hi in bands:
            bm = (bp_avg_rk_bk >= lo) & (bp_avg_rk_bk <= hi)
            if bm.sum() > 5:
                bch = sim_ch_bk[bm.index[bm]].values.flatten()
    ...
    bp_ly_true_arr = np.zeros((T_SIM, N_BP)) if return_extra else None
    for t in range(T_SIM):
        id_map = {eid: idx for idx, eid in enumerate(sim_ids[t])}
        for j, sid in enumerate(survivor_list):
            ...
            if return_extra:
                bp_ly_true_arr[t, j] = sim_ly_true[t, id_map[sid]]

    sim_df_bp = pd.DataFrame(bp_ly)
    sim_ch = sim_df_bp.diff().iloc[1:]
    sim_v1 = sim_ch.var()
    diag = {}

    for k in [2, 4, 8, 13]:
        if k < T_SIM:
            diag[f'vr{k}'] = (sim_df_bp.diff(k).iloc[k:].var() / (k * sim_v1)).median()
    ...
    # Fix #1: Persistence on endpoint IDs, not array positions
    for k in [1, 4, 13, 26, 52]:
        if k < T_SIM:
            t0_ids = set(sim_ids[0, sim_rk[0] <= 100])
            tk_ids = set(sim_ids[k, sim_rk[k] <= 100])
            diag[f'pers{k}'] = len(t0_ids & tk_ids)
    ...
    if return_extra:
        extra = {
            ...
            'bp_ly_true': bp_ly_true_arr, 'bp_rk': bp_rk,
            'sim_ch': sim_ch, 'sim_df': sim_df_bp, 'sim_v1': sim_v1,
            'sim_ch_flat': sim_ch_flat, 'xsec_vars': xsec_vars,
            'N_BP': N_BP, 'bp_avg_rk_global': bp_avg_rk_rep,
            'sim_rk_full': sim_rk,
            'sim_ly_full': sim_ly,
            'sim_ids_full': sim_ids,  # Fix #1: needed for ID-based persistence
        }
    return diag, extra


# ============================================================
# TWO-PASS KURTOSIS CALIBRATION (v3.9)
# ============================================================
# Fix #14: This is a heuristic calibration, not a principled estimator.
# It uses the analytical relationship between t-distribution df and excess
# kurtosis (kurt = 6/(df-4) for df>4) as a mapping to adjust df, but the
# actual kurtosis in the simulation is also affected by ARCH, het_multiplier,
# and clipping. The overshoot factor compensates for these nonlinearities.
print(f"\n{'='*70}")
print("KURTOSIS CALIBRATION PASS (v4.2)")
print(f"{'='*70}")
...
# Empirical band kurtosis targets
...
# Fix #7: Removed dead calibration loop. Use run_sim with return_band_kurtosis
# directly — no wasted reps.
print(f"\n  Running {N_CAL} calibration sims...")
cal_kurts_all = {(lo, hi): [] for lo, hi in bands}
for ci, cseed in enumerate(cal_seeds):
    t_cal = time.time()
    bk_result = run_sim(cseed, return_band_kurtosis=True)
...
for lo, hi in bands:
    ...
    if (lo, hi) in PROTECTED_BANDS:
        ...
        if sim_k > 0.5 and emp_k > 0.5 and abs(sim_k - emp_k) / emp_k > 0.10:
            ...
            new_df = np.clip(new_df, 4.2, 200.0)
...
for i, seed in enumerate(seeds):
    t_rep = time.time()
    diag, extra = run_sim(seed, return_extra=(seed == 42))
...
for k in [1, 4, 13, 26, 52]:
    if f'xr2_{k}' in mc_stats:
        ...
        ok = "Y" if err < 0.08 else "N"
        print(f"  R2({k:2d}): emp={xr2_emp[k]:.4f}  sim={s['mean']:.4f} [{s['lo']:.4f}, {s['hi']:.4f}]  err={err:.4f} [{ok}]")
...
tests = {
    ...
    'R2(1)': abs(mc_stats['xr2_1']['mean']-xr2_emp[1]) < 0.08,
    'R2(4)': abs(mc_stats['xr2_4']['mean']-xr2_emp[4]) < 0.08,
    'R2(13)': abs(mc_stats['xr2_13']['mean']-xr2_emp[13]) < 0.08,
}
n_pass = sum(tests.values())
elapsed = time.time() - t_start

print(f"\n{'='*70}")
print(f"SUMMARY v4.2 ({N_REP} replications)")
print(f"{'='*70}")
...
# ============================================================
# Fix #13: MC UNCERTAINTY REPORTING
# ============================================================
print(f"\n{'='*70}")
print(f"MC UNCERTAINTY ANALYSIS ({N_REP} replications)")
print(f"{'='*70}")

# For each diagnostic: MC SE, distance-to-threshold in SE units, fragility label
mc_diag_info = [
...
print(f"\n  {'Diagnostic':<12s} {'MC Mean':>8s} {'MC SE':>8s} {'Distance':>8s} {'Thresh':>8s} {'d/SE':>6s} {'Label':>8s}")
print("  " + "-" * 70)
for dname, dkey, emp_val, mode, thresh in mc_diag_info:
    vals = np.array([d[dkey] for d in all_diags])
    mc_mean = np.mean(vals)
    mc_se = np.std(vals) / np.sqrt(N_REP)
    if mode == 'rel':
        distance = thresh * emp_val - abs(mc_mean - emp_val)
    else:
        distance = thresh - abs(mc_mean - emp_val)
    d_over_se = distance / mc_se if mc_se > 1e-10 else float('inf')
    if d_over_se > 3:
        label = "solid"
    elif d_over_se > 1.5:
        label = "marginal"
        ...
        label = "FRAGILE"
    print(f"  {dname:<12s} {mc_mean:>8.4f} {mc_se:>8.4f} {distance:>8.4f} {thresh:>8.4f} {d_over_se:>6.1f} {label:>8s}")

# ============================================================
# PUBLICATION DIAGNOSTICS
# ============================================================
print(f"\n{'='*70}")
print("PUBLICATION DIAGNOSTICS (v4.2)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    sim_ids_full = rep_extra['sim_ids_full']  # Fix #1: for ID-based persistence

    # Fix #10: Seeded RNG for publication diagnostics
    pub_rng = np.random.RandomState(12345)
    ...
    # 1a. Anderson-Darling test
    sim_subsample = pub_rng.choice(sim_ch_flat, min(5000, len(sim_ch_flat)), replace=False)
    emp_subsample = pub_rng.choice(all_ch_emp, min(5000, len(all_ch_emp)), replace=False)
    ...
    print(f"    Interpretation: {'Cannot reject same distribution' if ad_sig > 0.05 else 'Distributions differ significantly'} at a=0.05")
    ...
    for lag_test in lb_lags_test:
        ...
        if n_tested > 0:
            ...
            print(f"    Lag {lag_test:2d}: {n_reject}/{n_tested} ({lb_reject_pct[lag_test]:.1f}%) reject at a=0.05")
    ...
    for label, data in [("Empirical", all_ch_emp), ("Simulated", sim_ch_flat)]:
        n_data = len(data)
        k_opt = int(np.sqrt(n_data))
        alpha_hill, se_hill = hill_estimator(data, k_opt)
        ci_lo = alpha_hill - 1.96 * se_hill
        ci_hi = alpha_hill + 1.96 * se_hill
        print(f"    {label}: a_hat={alpha_hill:.3f} +/- {1.96*se_hill:.3f} [{ci_lo:.3f}, {ci_hi:.3f}] (k={k_opt})")

    # 1e. Shorrocks Mobility Index
    ...
    sim_trans_13 = compute_transition_matrix(bp_rk, n_quintiles, 13)
    ...
    # Fix #1: Use endpoint IDs, not array positions
    ...
    for rk_lo, rk_hi, slabel in strata:
        t0_ids = set(sim_ids_full[0, (sim_rk_full[0] >= rk_lo) & (sim_rk_full[0] <= rk_hi)])
        n_in = len(t0_ids)
        if n_in == 0:
            continue
        half_life = None
        for k in range(1, T_SIM):
            tk_ids = set(sim_ids_full[k, (sim_rk_full[k] >= rk_lo) & (sim_rk_full[k] <= rk_hi)])
            frac = len(t0_ids & tk_ids) / n_in
            if frac < 0.5 and half_life is None:
                prev_k = k - 1
                tk_prev_ids = set(sim_ids_full[prev_k, (sim_rk_full[prev_k] >= rk_lo) & (sim_rk_full[prev_k] <= rk_hi)])
                frac_prev = len(t0_ids & tk_prev_ids) / n_in
                if frac_prev > frac:
        ...
        # Empirical (positions OK — no entry/exit in balanced panel)
    ...
    # 1h. Volatility clustering
    print(f"\n  Volatility Clustering (ACF of |changes| and changes^2):")
    emp_abs_acf = {}; sim_abs_acf = {}; emp_sq_acf = {}; sim_sq_acf = {}
    for lag in [1, 2, 4, 8]:
        ...
        print(f"    Lag {lag}: |dy| ACF: emp={emp_abs_acf[lag]:.4f} sim={sim_abs_acf[lag]:.4f}  "
              f"dy^2 ACF: emp={emp_sq_acf[lag]:.4f} sim={sim_sq_acf[lag]:.4f}")
    ...
    # === FIGURE 1: Core calibration diagnostics ===
    fig = plt.figure(figsize=(22, 28))
    gs = GridSpec(5, 3, figure=fig, hspace=0.35, wspace=0.30)
    fig.suptitle(f'Rank Diffusion v4.2 | {n_pass}/{len(tests)} | {N_REP} MC reps | '
    ...
    for k in [1,4,13]:
        ...
        if s: ax.errorbar(k, s['mean'], yerr=[[s['mean']-s['lo']],[s['hi']-s['mean']]],
                         fmt='b^', ms=4, capsize=3, alpha=0.7)
    ax.set_title('Cross-Sectional R-squared'); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # Persistence — Fix #1: use endpoint IDs
    ax = fig.add_subplot(gs[1,1])
    pk=[1,4,13,26,52]
    ax.plot(pk,[pers_emp.get(k,0) for k in pk],'ko-',label='Emp',ms=5,lw=2)
    sim_pers = [len(set(sim_ids_full[0, sim_rk_full[0]<=100]) &
                    set(sim_ids_full[k, sim_rk_full[k]<=100]))
    ...
    ax.plot(range(T_SIM), post_burnin_xsec[:T_SIM], 'r-', label='Sim tau', lw=2)
    ...
    plt.savefig(BASE_DIR / 'v42_diagnostics.png', dpi=130, bbox_inches='tight')
    print("Saved v42_diagnostics.png")
    plt.close()

    # === FIGURE 2: Publication diagnostics ===
    fig2 = plt.figure(figsize=(24, 32))
    gs2 = GridSpec(5, 3, figure=fig2, hspace=0.38, wspace=0.32)
    fig2.suptitle('Rank Diffusion v4.2 — Publication Diagnostics',
                  fontsize=13, fontweight='bold', y=0.995)

    # 2a. QQ Plot
    ax = fig2.add_subplot(gs2[0, 0])
    n_qq = min(10000, len(sim_std_resid_flat))
    qq_sample = np.sort(pub_rng.choice(sim_std_resid_flat, n_qq, replace=False))
    ...
    ax.plot(lims, lims, 'r-', lw=1.5, label='45 deg line')
    ...
    ax.set_xlabel('log10(Rank)'); ax.set_ylabel('log10(Value)')
    ax.set_title('Zipf Rank-Size Plot (log-log)')
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # 2c. Innovation density (log scale)
    ...
    ax.set_xlabel('|d log(y)|'); ax.set_ylabel('P(|d log(y)| > x)')
    ...
    ax.set_xlabel('k (order statistics)'); ax.set_ylabel('a_hat (tail index)')
    ax.set_title('Hill Plot: Tail Index vs k')
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ax.set_ylim(0, max(15, max(emp_hills.max(), sim_hills.max()) * 1.1))

    # 2h. Volatility clustering bars
    ax = fig2.add_subplot(gs2[2, 1])
    vol_lags = [1, 2, 4, 8]
    x_v = np.arange(len(vol_lags))
    w = 0.18
    ax.bar(x_v - 1.5*w, [emp_abs_acf[l] for l in vol_lags], w, label='|dy| Emp', color='black', alpha=0.7)
    ax.bar(x_v - 0.5*w, [sim_abs_acf[l] for l in vol_lags], w, label='|dy| Sim', color='red', alpha=0.7)
    ax.bar(x_v + 0.5*w, [emp_sq_acf[l] for l in vol_lags], w, label='dy^2 Emp', color='gray', alpha=0.7)
    ax.bar(x_v + 1.5*w, [sim_sq_acf[l] for l in vol_lags], w, label='dy^2 Sim', color='salmon', alpha=0.7)
    ax.axhline(0, color='gray', lw=0.5)
    ax.set_xticks(x_v); ax.set_xticklabels(vol_lags)
    ax.set_xlabel('Lag'); ax.set_ylabel('ACF')
    ax.set_title('Volatility Clustering: ACF of |dy| and dy^2')
    ax.legend(fontsize=7, ncol=2); ax.grid(True, alpha=0.3)

    # 2i. Kurtosis by band
    ax = fig2.add_subplot(gs2[2, 2])
    emp_kurts_band = []; sim_kurts_band = []; band_labels = []
    for lo, hi in bands:
    ...
    for ti, tc in zip(cdc_times, colors_cdc):
        ...
        ax.plot(rank_pct, cum_share * 100, color=tc, lw=1.5, alpha=0.8, label=f'Sim wk {ti}')
    ...
    # 2k. Survival curves — Fix #1: use endpoint IDs
    ...
    for top_k, ckm in zip(top_ks, colors_km):
        t0_top_ids = set(sim_ids_full[0, sim_rk_full[0] <= top_k])
        n_init = len(t0_top_ids)
        surv_sim = []
        for wk in range(T_SIM):
            tw_ids = set(sim_ids_full[wk, sim_rk_full[wk] <= top_k])
            surv_sim.append(len(t0_top_ids & tw_ids) / n_init * 100)
        ax.plot(range(T_SIM), surv_sim, color=ckm, lw=1.5, label=f'Sim top-{top_k}')
        # Empirical (balanced panel — positions OK)
    ...
    qq_emp_sample = np.sort(pub_rng.choice(z_within, n_qq_emp, replace=False))
    ...
    for k_rr, c_rr in [(1, 'navy'), (4, 'green'), (13, 'orange'), (26, 'red')]:
        if k_rr < T_SIM:
            ...
            ax.plot(bin_means_r0, bin_means_rk, '-', color=c_rr, lw=1.5, alpha=0.8, label=f'k={k_rr}')
    ...
    plt.savefig(BASE_DIR / 'v42_pub_diagnostics.png', dpi=130, bbox_inches='tight')
    print("Saved v42_pub_diagnostics.png")
    plt.close()
...
# ============================================================
print(f"\n\n{'='*70}")
print("PHASE 2: ABLATION STUDY (v4.2)")
print(f"{'='*70}")

# Fix #11: Harmonized to N_REP=25 (was 15 in v4.0)

# Ablation levels — configs for unified run_sim
ablation_levels = [
...
abl_seeds = [42] + list(range(100, 100 + N_REP - 1))
all_abl_results = []
...
for lvl_idx, level in enumerate(ablation_levels):
    ...
    for ri, sd in enumerate(abl_seeds):
        t0_abl = time.time()
        d, _ = run_sim(sd, config=level['config'])
        lvl_diags.append(d)
        if ri == 0 or ri == N_REP - 1:
            print(f"    Rep {ri+1}/{N_REP}: {time.time()-t0_abl:.1f}s")
...
plt.tight_layout()
plt.savefig(BASE_DIR / 'v42_ablation.png', dpi=200, bbox_inches='tight')
print(f"  Saved: v42_ablation.png")
plt.close()
...
# ============================================================
print(f"\n\n{'='*70}")
print("PHASE 3: PARAMETER SENSITIVITY ANALYSIS (v4.2)")
print(f"{'='*70}")

# Fix #11: Harmonized to N_REP=25 (was 10 in v4.0)
sens_seeds = [42] + list(range(300, 300 + N_REP - 1))
...
for pname, pvar, pbase in sens_params:
    ...
    for delta in perturbations:
        ...
        for sd in sens_seeds:
            d, _ = run_sim(sd, config=overrides)
            diags_list.append(d)
...
fig_sens, axes_sens = plt.subplots(2, 3, figsize=(16, 10))
...
plt.tight_layout()
plt.savefig(BASE_DIR / 'v42_sensitivity.png', dpi=200, bbox_inches='tight')
print(f"  Saved: v42_sensitivity.png")
plt.close()
...
print(f"  Phase 3 — Sensitivity: {len(sens_params)} params x {len(perturbations)} perturbations")
