#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v3.1
===============================================
Fixes from v3.0:

v3.0 achieved 14/15. Only failure: Pers(13)=47 vs emp 64 (diff -17).

Root cause: The 50-week burn-in creates excessive transients. New entries
during burn-in are assigned type=1 (transient, 7% exit rate), generating a
snowball of high-turnover endpoints. After burn-in, the population is 40%+
transients, causing exit rate 269/wk (target 198) and survivor rate 59%
(target 71%). The excess churn reduces top-100 persistence.

v3.1 fix: DISABLE exit/entry during burn-in. The purpose of burn-in is only
to let the transitory component c reach its stationary distribution. Population
dynamics (exit/entry) should only start at recording time.

All other parameters unchanged from v3.0.
"""
...
fig.suptitle(f'Rank Diffusion v3.1 | {n_pass}/{len(tests)} | σ_obs={sigma_obs:.3f} κ={kappa:.5f} t_df={t_df:.1f} (all estimated)',
...
plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v31_diagnostics.png', dpi=130, bbox_inches='tight')
print("Saved v31_diagnostics.png")
print("Done.")
