#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v3.6 — ARCH(1) Volatility Clustering
================================================================================
Changes from v3.5 (15/15, publication diagnostics):

v3.6 addresses the most important gap revealed by v3.5's publication diagnostics:
volatility clustering. The model captured only 41% of the empirical lag-1 ACF
of absolute changes (sim=0.111 vs emp=0.270).

Key change: ARCH(1) on the transitory innovation
  σ²_{ν,i,t} = σ²_{ν,base} × [(1-α_arch) + α_arch × z²_{i,t-1}]

  where z²_{i,t-1} = ν²_{i,t-1} / E[ν²_i] is the normalized squared innovation.

  After a large shock, the next period's transitory innovation variance increases,
  creating volatility clustering. The unconditional E[σ²_{ν,i,t}] is preserved
  (E[z²] = 1), so unconditional variance, VR, ACF, RACF, R², and Pers are all
  unaffected in expectation. Only second-moment dynamics change.

  α_arch is estimated from within-endpoint autocorrelation of squared standardized
  residuals.

All v3.5 publication diagnostics retained.
"""
...
print("\n" + "=" * 70)
print("PARAMETER SUMMARY (v3.6: v3.4 engine + ARCH(1) + pub diagnostics)")
print("=" * 70)
print(f"  σ_obs     = {sigma_obs:.4f}")
...
# ============================================================
# SIMULATION (v3.4 engine + ARCH(1) on transitory innovation)
# ============================================================
T_SIM = n_weeks
T_BURNIN = 50
...
print(f"\n{'='*70}")
print(f"SIMULATION v3.6 — {N_REP} MC REPS — ARCH(1) α={alpha_arch:.4f}")
print(f"{'='*70}")
...
def run_simulation(seed):
    ...
    for t_abs in range(1, T_TOTAL):
        ...
        t_scale = sn_het * np.sqrt(max(t_df-2, 0.5)/t_df) * arch_scale
        ...
        # E[ν²] = sn_het² (by construction of t_scale without arch_scale)
...
print(f"\n{'='*70}")
print(f"SUMMARY v3.6 ({N_REP} replications)")
print(f"{'='*70}")
...
# ============================================================
print(f"\n{'='*70}")
print("PUBLICATION DIAGNOSTICS (v3.6)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    fig.suptitle(f'Rank Diffusion v3.6 | {n_pass}/{len(tests)} | {N_REP} MC reps | '
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v36_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v36_diagnostics.png")
    plt.close()
    ...
    fig2.suptitle('Rank Diffusion v3.6 — Publication Diagnostics',
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v36_pub_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v36_pub_diagnostics.png")
    plt.close()

# Final summary
print(f"\n{'='*70}")
print(f"v3.6 COMPLETE")
print(f"{'='*70}")
...
print(f"              α_arch={alpha_arch:.4f} (NEW: ARCH(1) on transitory)")
print(f"  Key change: ARCH(1) volatility clustering on transitory innovation")
