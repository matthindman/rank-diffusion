#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v3.3
===============================================
Changes from v3.1 (14/15) and v3.2 (8/15, regression):

v3.2 FAILED: Rank-local mean reversion (pulling toward Zipf target at current
rank) stabilized endpoints at their "correct" positions, making RACF and R²
worse. Pers(13) barely changed. The rank-local approach was fundamentally
wrong — it reduces displacement rather than increasing it.

v3.3 strategy: Return to v3.1's proven global-mean architecture, but add a
RANK-DEPENDENT κ MULTIPLIER that makes mean reversion weak for top ranks
and full strength for the bulk. This preserves variance control while removing
the systematic downward drag on top endpoints.
...
# v3.3: Rank-dependent global-mean reversion
# κ(r) = κ_base × (r/N)^α
...
N_FULL = int(mean_N)
alpha_kappa = 0.3  # Controls how much top ranks are shielded
...
print(f"\n{'='*70}")
print(f"SIMULATION v3.3 — {N_REP} MONTE CARLO REPLICATIONS")
print(f"{'='*70}")
...
print(f"\n{'='*70}")
print(f"SUMMARY v3.3 ({N_REP} replications)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    fig.suptitle(f'Rank Diffusion v3.3 | {n_pass}/{len(tests)} | {N_REP} MC reps | '
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v33_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v33_diagnostics.png")
