#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v3.8 — Two-Pass Kurtosis Calibration
================================================================================
Changes from v3.7 (15/15, rank-dependent t_df + ARCH(1)):

v3.7 introduced rank-dependent t_df per band, bringing most bands close to
empirical kurtosis targets, but a gap remains for band 501-2K:
  - 501-2K:  sim=4.58 vs emp=7.44 (gap: 2.86 units)
  - 1-100:   sim=2.74 vs emp=1.77 (small overshoot, 0.97 units)
  - Other bands: nearly perfect

Root cause: The MLE t_df captures the *marginal* tail shape, but the simulation's
realized kurtosis is a nonlinear function of t_df, ARCH amplification, φ (AR(1)
accumulation), and heterogeneity. A single-pass MLE cannot account for these
interactions.

Key change: Two-pass kurtosis calibration
  - Pass 1: Run 5 quick calibration replications with MLE-based t_df (same as v3.7)
  - Measure realized band-level kurtosis from calibration runs
  - Compute adjustment ratio: new_df adjusted via the relationship
    kurt ∝ 6/(df-4) for t-distribution, scaled by the simulation amplification
  - Pass 2: Run full 25-rep MC with calibrated t_df values

Why this is safe:
  - t_df only affects innovation *shape*, not *variance* (variance-normalized)
  - VR, ACF, RACF, R², Pers depend on variance/covariance structure -> unchanged
  - The calibration targets the same empirical kurtosis values, just more accurately

All v3.7 features retained: rank-dependent t_df, ARCH(1), publication diagnostics.
"""
...
print("\n" + "=" * 70)
print("PARAMETER SUMMARY (v3.8: v3.7 + two-pass kurtosis calibration)")
print("=" * 70)
print(f"  σ_obs     = {sigma_obs:.4f}")
...
print(f"\n{'='*70}")
print(f"SIMULATION v3.8 — {N_REP} MC REPS — ARCH(1) + calibrated rank-dep t_df")
print(f"{'='*70}")
...
def run_simulation(seed):
    ...
    # v3.8: Band-level kurtosis for every rep (for reliable 25-rep averaging)
...
# ============================================================
# TWO-PASS KURTOSIS CALIBRATION (new in v3.8)
# ============================================================
...
print(f"\n{'='*70}")
print("KURTOSIS CALIBRATION PASS (v3.8)")
print(f"{'='*70}")
...
print(f"\n{'='*70}")
print(f"SUMMARY v3.8 ({N_REP} replications)")
print(f"{'='*70}")
...
# ============================================================
print(f"\n{'='*70}")
print("PUBLICATION DIAGNOSTICS (v3.8)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    fig.suptitle(f'Rank Diffusion v3.8 | {n_pass}/{len(tests)} | {N_REP} MC reps | '
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v38_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v38_diagnostics.png")
    plt.close()
    ...
    fig2.suptitle('Rank Diffusion v3.8 — Publication Diagnostics',
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v38_pub_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v38_pub_diagnostics.png")
    plt.close()

# Final summary
print(f"\n{'='*70}")
print(f"v3.8 COMPLETE")
print(f"{'='*70}")
...
print(f"              α_arch={alpha_arch:.4f}")
print(f"  Key change: Two-pass kurtosis calibration (bands 501+ only)")
