#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v4.0 — Ablation Study + Param Sensitivity
================================================================================
Changes from v3.9 (15/15, cross-sectional variance stabilization):

v4.0 adds two publication-critical analyses that directly address the most likely
peer-review critiques of the modeling approach:

1. ABLATION STUDY (addresses "overfitting to diagnostic suite" critique):
   Builds up from a minimal PT+Gaussian model to the full v3.9 in 8 levels,
   evaluating 15 diagnostics at each step. Key finding: only 3 features beyond
   the base PT decomposition affect pass/fail (burn-in, κ, rank-dep κ); the
   distributional features (tails, ARCH, rank-dep t_df) improve kurtosis/
   volatility clustering but change zero pass/fail outcomes; the final κ-stab
   actually *worsens* the calibration score in exchange for better stationarity.

2. PARAMETER SENSITIVITY ANALYSIS (addresses "identification" critique):
   Perturbs each key parameter by ±10% and ±20% and reports which diagnostics
   break, revealing the identification structure of the model.

All v3.9 features retained. The core simulation is identical to v3.9.
"""
...
print("\n" + "=" * 70)
print("PARAMETER SUMMARY (v4.0: v3.9 + ablation + param sensitivity)")
print("=" * 70)
print(f"  σ_obs     = {sigma_obs:.4f}")
...
print(f"\n{'='*70}")
print(f"SIMULATION v4.0 — {N_REP} MC REPS — ARCH(1) + calibrated rank-dep t_df")
print(f"{'='*70}")
...
def run_simulation(seed):
    ...
    # v3.9: Band-level kurtosis for every rep (for reliable 25-rep averaging)
...
print(f"\n{'='*70}")
print("KURTOSIS CALIBRATION PASS (v4.0)")
print(f"{'='*70}")
...
def measure_band_kurtosis(seed_val):
    """Run a simulation and return band-level kurtosis."""
    ...
    ep_type2 = np.zeros(N_FULL, dtype=int)
    endpoint_id2 = np.arange(N_FULL)
    next_id2 = N_FULL
    last_z_sq2 = np.ones(N_FULL)

    # Collect balanced panel changes
    sim_ly2 = np.zeros((T_SIM, N_FULL))
    sim_ids2 = np.zeros((T_SIM, N_FULL), dtype=int)
    sim_rk2 = np.zeros((T_SIM, N_FULL), dtype=int)
    obs_noise2 = rng2.normal(0, sigma_obs, N_FULL)
    ...
    for t_abs2 in range(1, T_TOTAL):
        ...
        if t_rec2 >= 0:
            nr2 = cr2 / N_FULL
            p_exit2 = np.where(ep_type2 == 0, inc_p_base * (nr2 ** inc_alpha), trans_p_exit)
            exit_mask2 = rng2.random(N_FULL) < p_exit2
            n_ex2 = exit_mask2.sum()
            if n_ex2 > 0:
                exi2 = np.where(exit_mask2)[0]
                n_burst2 = max(1, int(n_ex2 * 0.008))
                n_norm2 = n_ex2 - n_burst2
                bq2 = np.percentile(tau2[~exit_mask2], 10)
                bstd2 = np.std(tau2[tau2 < np.median(tau2)]) * 0.4
                new_tau2 = rng2.normal(bq2, bstd2, n_norm2)
                if n_burst2 > 0:
                    buq2 = np.percentile(tau2[~exit_mask2], 90)
                    bust2 = np.std(tau2) * 0.25
                    new_tau2 = np.concatenate([new_tau2, rng2.normal(buq2, bust2, n_burst2)])
                tau2[exi2] = new_tau2
                c_state2[exi2] = sp_stats.t.rvs(df=t_df, size=n_ex2, random_state=rng2) * 0.3
                het_mul2[exi2] = np.clip(np.exp(rng2.normal(0, sigma_het, n_ex2)), 0.15, 8.0)
                last_z_sq2[exi2] = 1.0
                ep_type2[exi2] = 1
                endpoint_id2[exi2] = np.arange(next_id2, next_id2+n_ex2)
                next_id2 += n_ex2
        log_y_obs2 = tau2 + c_state2 + rng2.normal(0, sigma_obs, N_FULL)
        order2 = np.argsort(-np.exp(log_y_obs2))
        ranks2 = np.empty(N_FULL, dtype=int); ranks2[order2] = np.arange(1, N_FULL+1)
        if t_rec2 == 0:
            sim_ly2[0] = log_y_obs2; sim_rk2[0] = ranks2; sim_ids2[0] = endpoint_id2.copy()
        elif 0 < t_rec2 < T_SIM:
            sim_ly2[t_rec2] = log_y_obs2; sim_rk2[t_rec2] = ranks2; sim_ids2[t_rec2] = endpoint_id2.copy()

    # Balanced panel
    init_ids2 = set(sim_ids2[0])
    survivors2 = init_ids2.copy()
    for t2 in range(1, T_SIM):
        survivors2 &= set(sim_ids2[t2])
    survivor_list2 = sorted(survivors2)
    N_BP2 = len(survivor_list2)
    bp_ly2 = np.zeros((T_SIM, N_BP2))
    bp_rk2 = np.zeros((T_SIM, N_BP2), dtype=int)
    for t2 in range(T_SIM):
        id_map2 = {eid: idx for idx, eid in enumerate(sim_ids2[t2])}
        for j2, sid2 in enumerate(survivor_list2):
            bp_ly2[t2, j2] = sim_ly2[t2, id_map2[sid2]]
            bp_rk2[t2, j2] = sim_rk2[t2, id_map2[sid2]]
...
print(f"\n{'='*70}")
print(f"SUMMARY v4.0 ({N_REP} replications)")
print(f"{'='*70}")
...
# ============================================================
print(f"\n{'='*70}")
print("PUBLICATION DIAGNOSTICS (v4.0)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v40_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v40_diagnostics.png")
    plt.close()
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v40_pub_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v40_pub_diagnostics.png")
    plt.close()
...
def run_ablation_sim(seed, config):
    ...
    ep_type_a = np.zeros(N_FULL, dtype=int)
    eid_a = np.arange(N_FULL)
    nxt_a = N_FULL
    lzs_a = np.ones(N_FULL)

    sim_ly_a = np.zeros((T_SIM, N_FULL))
    sim_rk_a = np.zeros((T_SIM, N_FULL), dtype=int)
    sim_ids_a = np.zeros((T_SIM, N_FULL), dtype=int)

    obs_a = rng.normal(0, sigma_obs, N_FULL)
    ...
    if t_burnin == 0:
        sim_ly_a[0] = y0_a; sim_rk_a[0] = rk_a; sim_ids_a[0] = eid_a.copy()
    ...
    for t_abs in range(1, t_total):
        ...
        if t_rec >= 0:
            nr = cr / N_FULL
            pe = np.where(ep_type_a == 0, inc_p_base * (nr ** inc_alpha), trans_p_exit)
            em = rng.random(N_FULL) < pe
            ne = em.sum()
            tot_ex_a += ne
            if ne > 0:
                exi = np.where(em)[0]
                nb = max(1, int(ne * 0.008)); nn = ne - nb
                bq = np.percentile(tau_a[~em], 10)
                bs = np.std(tau_a[tau_a < np.median(tau_a)]) * 0.4
                nt = rng.normal(bq, bs, nn)
                if nb > 0:
                    buq = np.percentile(tau_a[~em], 90)
                    bust = np.std(tau_a) * 0.25
                    nt = np.concatenate([nt, rng.normal(buq, bust, nb)])
                tau_a[exi] = nt
                c_a[exi] = rng.normal(0, 0.3, ne)
                het_a[exi] = np.clip(np.exp(rng.normal(0, sigma_het, ne)), 0.15, 8.0)
                lzs_a[exi] = 1.0
                ep_type_a[exi] = 1
                eid_a[exi] = np.arange(nxt_a, nxt_a+ne); nxt_a += ne

        ly_obs = tau_a + c_a + rng.normal(0, sigma_obs, N_FULL)
        ord_a = np.argsort(-np.exp(ly_obs))
        rk_a = np.empty(N_FULL, dtype=int); rk_a[ord_a] = np.arange(1, N_FULL+1)

        if t_rec == 0:
            sim_ly_a[0] = ly_obs; sim_rk_a[0] = rk_a; sim_ids_a[0] = eid_a.copy()
        elif 0 < t_rec < T_SIM:
            sim_ly_a[t_rec] = ly_obs; sim_rk_a[t_rec] = rk_a; sim_ids_a[t_rec] = eid_a.copy()

    # Balanced panel
    init_ids = set(sim_ids_a[0])
    surv = init_ids.copy()
    for t in range(1, T_SIM):
        surv &= set(sim_ids_a[t])
    sl = sorted(surv); nbp = len(sl)

    bp_ly_a = np.zeros((T_SIM, nbp)); bp_rk_a = np.zeros((T_SIM, nbp), dtype=int)
    for t in range(T_SIM):
        idm = {eid: idx for idx, eid in enumerate(sim_ids_a[t])}
        for j, sid in enumerate(sl):
            bp_ly_a[t, j] = sim_ly_a[t, idm[sid]]
            bp_rk_a[t, j] = sim_rk_a[t, idm[sid]]
...
plt.tight_layout()
plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v40_ablation.png',
            dpi=200, bbox_inches='tight')
print(f"  Saved: v40_ablation.png")
plt.close()
...
def run_sensitivity_sim(seed, overrides):
    ...
    ep_t_s = np.zeros(N_FULL, dtype=int)
    eid_s = np.arange(N_FULL)
    nxt_s = N_FULL
    ...
    sim_ids_s = np.zeros((T_SIM, N_FULL), dtype=int)

    obs_s = rng.normal(0, s_obs, N_FULL)
    ...
    for t_abs in range(1, t_total):
        ...
        if t_rec >= 0:
            nr = cr / N_FULL
            pe = np.where(ep_t_s == 0, inc_p_base * (nr ** inc_alpha), trans_p_exit)
            em = rng.random(N_FULL) < pe
            ne = em.sum()
            if ne > 0:
                exi = np.where(em)[0]
                nb = max(1, int(ne * 0.008)); nn = ne - nb
                bq = np.percentile(tau_s[~em], 10)
                bs = np.std(tau_s[tau_s < np.median(tau_s)]) * 0.4
                nt = rng.normal(bq, bs, nn)
                if nb > 0:
                    buq = np.percentile(tau_s[~em], 90)
                    bust = np.std(tau_s) * 0.25
                    nt = np.concatenate([nt, rng.normal(buq, bust, nb)])
                tau_s[exi] = nt
                c_s[exi] = sp_stats.t.rvs(df=tdf_g, size=ne, random_state=rng) * 0.3
                het_s[exi] = np.clip(np.exp(rng.normal(0, s_het, ne)), 0.15, 8.0)
                lzs_s[exi] = 1.0
                ep_t_s[exi] = 1
                eid_s[exi] = np.arange(nxt_s, nxt_s+ne); nxt_s += ne

        ly_obs = tau_s + c_s + rng.normal(0, s_obs, N_FULL)
        ord_s = np.argsort(-np.exp(ly_obs))
        rk_s = np.empty(N_FULL, dtype=int); rk_s[ord_s] = np.arange(1, N_FULL+1)

        if t_rec == 0:
            sim_ly_s[0] = ly_obs; sim_rk_s[0] = rk_s; sim_ids_s[0] = eid_s.copy()
        elif 0 < t_rec < T_SIM:
            sim_ly_s[t_rec] = ly_obs; sim_rk_s[t_rec] = rk_s; sim_ids_s[t_rec] = eid_s.copy()

    init_ids = set(sim_ids_s[0])
    surv = init_ids.copy()
    for t in range(1, T_SIM):
        surv &= set(sim_ids_s[t])
    sl = sorted(surv); nbp = len(sl)
    bp_ly_s = np.zeros((T_SIM, nbp)); bp_rk_s = np.zeros((T_SIM, nbp), dtype=int)
    for t in range(T_SIM):
        idm = {eid: idx for idx, eid in enumerate(sim_ids_s[t])}
        for j, sid in enumerate(sl):
            bp_ly_s[t, j] = sim_ly_s[t, idm[sid]]
            bp_rk_s[t, j] = sim_rk_s[t, idm[sid]]
...
plt.tight_layout()
plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v40_sensitivity.png',
            dpi=200, bbox_inches='tight')
print(f"  Saved: v40_sensitivity.png")
plt.close()
