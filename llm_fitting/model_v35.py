#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v3.5 — Publication Diagnostics
========================================================================
Changes from v3.4 (15/15 all diagnostics pass):

v3.5 keeps v3.4's simulation engine EXACTLY UNCHANGED. All 15 calibration
diagnostics remain. This version adds comprehensive publication-standard
diagnostics, plots, and formal statistical tests commonly expected in
high-profile publications on rank-based dynamical systems.

New diagnostics added:
  Plots:
    - QQ plot of innovations vs fitted t-distribution
    - Log-log Zipf rank-size plot (empirical vs simulated)
    - Innovation density on log y-scale (tail visibility)
    - Rank transition heatmap (quintile-to-quintile)
    - CCDF on log-log axes (empirical vs model tails)
    - ACF of |changes| and changes² (volatility clustering)
    - Hill plot for tail index estimation
    - Kurtosis by rank band (empirical vs simulated)
    - Capital distribution curves (CDC) at multiple dates
    - Kaplan-Meier survival curves for top-K persistence

  Formal tests:
    - Anderson-Darling test (tail-sensitive GoF)
    - Ljung-Box test (residual serial correlation)
    - Jarque-Bera test (normality of standardized residuals)
    - Hill estimator with 95% CI
    - Shorrocks mobility index from transition matrix
...
print("\n" + "=" * 70)
print("PARAMETER SUMMARY (v3.5 = v3.4 engine + pub diagnostics)")
print("=" * 70)
print(f"  σ_obs     = {sigma_obs:.4f}")
...
print(f"\n{'='*70}")
print(f"SIMULATION v3.5 — {N_REP} MONTE CARLO REPLICATIONS")
print(f"{'='*70}")
...
print(f"\n{'='*70}")
print(f"SUMMARY v3.5 ({N_REP} replications)")
print(f"{'='*70}")
...
# ============================================================
print(f"\n{'='*70}")
print("PUBLICATION DIAGNOSTICS (v3.5)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    fig.suptitle(f'Rank Diffusion v3.5 | {n_pass}/{len(tests)} | {N_REP} MC reps | '
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v35_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v35_diagnostics.png")
    plt.close()
    ...
    fig2.suptitle('Rank Diffusion v3.5 — Publication Diagnostics',
    ...
    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v35_pub_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("Saved v35_pub_diagnostics.png")
    plt.close()

# Final summary
print(f"\n{'='*70}")
print(f"v3.5 COMPLETE")
print(f"{'='*70}")
...
print(f"  New diagnostics: AD test, JB test, LB test, Hill estimator,")
print(f"                   Shorrocks index, half-life by stratum, kurtosis by band,")
print(f"                   volatility clustering ACF")
print(f"  New plots: QQ, Zipf, log-density, transition heatmaps, CCDF,")
print(f"             Hill plot, vol clustering, kurtosis by band, CDC,")
print(f"             KM survival, rank-rank scatter, density snapshots,")
print(f"             Shorrocks vs horizon")
