#!/usr/bin/env python3
"""
Permanent-Transitory Rank Diffusion Model v2.7
===============================================
Key change from v2.6: PRINCIPLED PARAMETER ESTIMATION

All simulation parameters estimated from data (no hand-tuning):

1. σ_obs: Analytical estimate from ACF structure at lags 1-3.
   For the PT+noise model, γ(h) = -σ²_c(1-φ)²φ^{h-1} for h≥2,
   but γ(1) = -σ²_c(1-φ)² - σ²_obs. Only lag-1 is affected by obs noise.
   So: φ_agg = ρ(3)/ρ(2), then σ²_obs = -γ(1) + γ(2)/φ_agg.

2. σ_het: From cross-endpoint variance heterogeneity.
   If h_i ~ lognormal(0, σ_het), then mean(var_i)/median(var_i) ≈ exp(2σ²_het).
   So: σ_het = sqrt(log(mean_var/median_var) / 2).

3. t_df: MLE fit of scaled t-distribution to empirical changes.

4. κ: Cross-sectional variance stationarity condition.
   d/dt Var(τ) = E[h²]·mean(σ²_η) - 2κ·Var(τ) = 0
   So: κ = E[h²]·mean(σ²_η) / (2·Var_xsec).

5. perm_boost = 1.0 (eliminated — trust estimated parameters).

6. jump_prob, jump_scale: From tail excess beyond fitted t-distribution.

Band-level (σ_η, φ, σ_ν) still estimated via moment matching (as in v2.6).
"""
...
# ============================================================
print("\n" + "=" * 70)
print("STAGE 3: ESTIMATE t_df FROM CHANGE DISTRIBUTION (MLE)")
print("=" * 70)

# Fit a scaled t-distribution to the standardized changes
z_changes = (all_ch_emp - np.mean(all_ch_emp)) / np.std(all_ch_emp)
df_fit, loc_fit, scale_fit = sp_stats.t.fit(z_changes)
t_df = max(3.0, df_fit)  # Floor at 3.0

# The MLE df captures the OVERALL tail behavior. Since the changes
# are a mixture of Gaussian (permanent + obs noise) and t-distributed
# (transitory) components, the actual t_df for innovations should
# be somewhat lower (the Gaussian components dilute the tails).
# Apply a correction factor based on the transitory variance fraction.
trans_frac = 1.0 - sigma2_obs_est / emp_median_var - max(0, sigma2_eta_agg) / emp_median_var
t_df_corrected = max(3.0, t_df * trans_frac + 3.0 * (1 - trans_frac))

print(f"  MLE t-fit: df={df_fit:.2f}, loc={loc_fit:.4f}, scale={scale_fit:.4f}")
print(f"  Transitory variance fraction: {trans_frac:.3f}")
print(f"  Corrected t_df: {t_df_corrected:.2f} (raw MLE: {df_fit:.2f})")
print(f"  Using t_df = {t_df:.2f}")
...
threshold = 4.0
expected_tail = 2 * sp_stats.t.sf(threshold, df=t_df, loc=0, scale=1)
actual_tail = np.mean(np.abs(z_changes) > threshold)
jump_prob = max(0.005, actual_tail - expected_tail)

# Jump scale from the ratio of extreme-change magnitude to typical magnitude
extreme_mask = np.abs(z_changes) > threshold
if extreme_mask.sum() > 10:
    jump_scale = np.std(z_changes[extreme_mask]) / np.std(z_changes[~extreme_mask])
else:
    jump_scale = 5.0
...
# PARAMETER SUMMARY
# ============================================================
perm_boost = 1.0  # No boost — trust estimated parameters
...
print(f"  perm_boost= {perm_boost:.1f}    (eliminated)")
...
print(f"  perm_boost={perm_boost}, jump_prob={jump_prob:.4f}, jump_scale={jump_scale:.2f}")
...
fig.suptitle(f'Rank Diffusion v2.7 | {n_pass}/{len(tests)} | σ_obs={sigma_obs:.3f} κ={kappa:.5f} t_df={t_df:.1f} (all estimated)',
...
plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/v27_diagnostics.png', dpi=130, bbox_inches='tight')
print("Saved v27_diagnostics.png")
print("Done.")
