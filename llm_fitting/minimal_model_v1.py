#!/usr/bin/env python3
"""
Minimal Permanent-Transitory Rank Diffusion Model — v1
======================================================
A parsimonious specification capturing the essential dynamics of ranked
platform ecosystems with near-minimum complexity.

DESIGN PHILOSOPHY
-----------------
The full model (v3.9) achieves 15/15 calibration diagnostics with 14 versions
of iterative refinement.  This minimal model asks: what is the SMALLEST set
of ingredients needed for a "very good" fit?

We analysed the iteration log (v2.6–v3.9) to identify the causal chain from
each component to the diagnostics it unlocks:

  Component                       Diagnostics unlocked         First version
  ─────────────────────────────────────────────────────────────────────────────
  Perm-trans decomposition (τ+c)  Foundational                 v2.6  (9/15)
  σ_obs from ACF lag structure    VR at all horizons           v2.7  (→VR pass)
  Within-endpoint t_df            Kurtosis fix (48.8→4.9)      v2.8
  σ_het (log-normal)              Kurtosis decomposition       v2.7
  50-week burn-in                 RACF breakthrough            v2.9  (→11/15)
  Global-mean κ + burn-in         R² pass                      v3.0  (→14/15)
  No exit/entry during burn-in    Survivors match              v3.1  (→14/15)
  Rank-dep κ: (r/N)^0.5          Pers(13) pass                v3.4  (→15/15)
  Band-level (σ_η, φ, σ_ν)       VR/ACF per rank band         v2.7+
  Jump component                  Tail excess beyond t-dist    v2.6+
  Exit/entry dynamics             Population realism           v2.6+

INCLUDED (10 ingredients):
  1. Permanent-transitory decomposition (τ + c with AR(1) transitory)
  2. Observation noise σ_obs (from ACF lag structure)
  3. Log-normal heterogeneity σ_het
  4. Band-level structural parameters (σ_η, φ, σ_ν) — 5 rank bands
  5. Student-t innovations with single global df
  6. Rank-dependent mean reversion: κ(r) = κ_base × (r/N)^0.5
  7. 50-week burn-in (no exit/entry during burn-in)
  8. Merton jump component (estimated from tail excess)
  9. Exit/entry population dynamics (during recording period only)
  10. Monte Carlo (25 replications)

EXCLUDED (4 refinements from v3.5–v3.9, diminishing returns):
  - ARCH(1) volatility clustering (v3.6)
    ↳ Improves secondary vol-clustering metric from 0% to ~50% of target.
      Adds kurtosis/R² tension.  Not a core calibration diagnostic.
  - Rank-dependent t_df with obs-noise correction (v3.7)
    ↳ Improves band-level kurtosis and top-100 half-life.
      A refinement; aggregate kurtosis is adequate with global t_df.
  - Two-pass kurtosis calibration (v3.8)
    ↳ Fine-tunes 501-2K band kurtosis via simulation feedback loop.
  - κ variance-stabilization factor (v3.9)
    ↳ Addresses cross-sectional variance drift; widens R² margin.

EXPECTED PERFORMANCE: 15/15 core diagnostics (matching v3.4).
  Weaker on: volatility clustering, band-level kurtosis, top-100 half-life,
...
# ============================================================
# STAGE 1: ESTIMATE σ_obs FROM ACF STRUCTURE
# ============================================================
print("\n" + "=" * 70)
print("STAGE 1: σ_obs FROM ACF LAG STRUCTURE")
print("=" * 70)
...
# ============================================================
# STAGE 2: ESTIMATE σ_het
# ============================================================
print("\n" + "=" * 70)
print("STAGE 2: σ_het FROM MEAN/MEDIAN VARIANCE RATIO")
print("=" * 70)
...
# ============================================================
# STAGE 3: GLOBAL t_df FROM WITHIN-ENDPOINT RESIDUALS
# ============================================================
print("\n" + "=" * 70)
print("STAGE 3: GLOBAL t_df (WITHIN-ENDPOINT MLE)")
print("=" * 70)

# Standardize each endpoint's changes by its own mean/std, pool, then MLE.
# This removes between-endpoint heterogeneity that would bias t_df downward.
...
t_df = max(3.0, df_fit)
print(f"  MLE on within-endpoint standardized residuals: df={df_fit:.2f}")
print(f"  t_df = {t_df:.2f}")

# ============================================================
# STAGE 4: JUMP PARAMETERS FROM TAIL EXCESS
# ============================================================
print("\n" + "=" * 70)
...
jump_scale = (np.std(z_within[extreme_mask]) / np.std(z_within[~extreme_mask])
              if extreme_mask.sum() > 10 else 5.0)
...
# ============================================================
print("\n" + "=" * 70)
print("STAGE 5: BAND-LEVEL STRUCTURAL PARAMETERS")
print("=" * 70)

def model_vr(k, se2, phi, sn2, sobs2=0):
    """Analytical variance ratio VR(k) from the perm-trans model."""
...
def model_acf1_fn(se2, phi, sn2, sobs2=0):
    """Analytical ACF(1) from the perm-trans model."""
    sc2 = sn2 / (1 - phi ** 2) if abs(phi) < 0.999 else sn2 * 1000
    vd = se2 + 2 * sc2 * (1 - phi) + 2 * sobs2
    if vd <= 0: return 0.0
...
def fit_params(emp_var, emp_vr4, emp_acf1, emp_vr13=None, sobs2=0):
    """Estimate (σ²_η, φ, σ²_ν) by matching band-level moments."""
    def objective(p):
...
for (lo, hi), st in band_stats.items():
    ...
    sc2 = sn2/(1-phi**2)
    mvar_t = se2 + 2*sc2*(1-phi) + 2*sobs2
    band_params[(lo,hi)] = {'se':np.sqrt(se2),'phi':phi,'sn':np.sqrt(sn2),
                            'se2':se2,'sn2':sn2,'pf':se2/mvar_t}
    print(f"  {lo:5d}-{hi:5d}: σ_η={np.sqrt(se2):.4f}  φ={phi:.4f}  "
          f"σ_ν={np.sqrt(sn2):.4f}  perm={se2/mvar_t*100:.1f}%")
...
def get_p(ranks):
    """Interpolate (σ_η, φ, σ_ν) as function of rank via log-rank."""
...
# ============================================================
# STAGE 6: RANK-DEPENDENT κ CALIBRATION
# ============================================================
print("\n" + "=" * 70)
print("STAGE 6: RANK-DEPENDENT κ")
print("=" * 70)

N_FULL = int(mean_N)
alpha_kappa = 0.5  # Power-law exponent — shields top ranks

# Analytical stationarity condition: κ_base = E[η²] / (2 · E[weight · dev²])
...
# ============================================================
print("\n" + "=" * 70)
print("MINIMAL MODEL — PARAMETER SUMMARY")
print("=" * 70)
print(f"  σ_obs     = {sigma_obs:.4f}   (from ACF lag structure)")
print(f"  σ_het     = {sigma_het:.4f}   (from mean/median variance ratio)")
print(f"  t_df      = {t_df:.2f}     (global; within-endpoint MLE)")
print(f"  κ_base    = {kappa_base:.6f} (analytical stationarity)")
print(f"  α_κ       = {alpha_kappa}      (rank-dependent power law)")
print(f"  T_burnin  = 50 weeks")
print(f"  jump_prob = {jump_prob:.4f}")
print(f"  jump_scale= {jump_scale:.2f}")
se_str = ', '.join(f'{p["se"]:.4f}' for p in band_params.values())
phi_str = ', '.join(f'{p["phi"]:.4f}' for p in band_params.values())
sn_str = ', '.join(f'{p["sn"]:.4f}' for p in band_params.values())
print(f"  Band σ_η:   {se_str}")
print(f"  Band φ:     {phi_str}")
print(f"  Band σ_ν:   {sn_str}")
...
# Variance normalization for unit-variance t innovations
t_var_factor = np.sqrt(max(t_df - 2, 0.5) / t_df)

print(f"\n{'='*70}")
print(f"SIMULATION — {N_REP} MC REPLICATIONS")
print(f"{'='*70}")
print(f"  N={N_FULL}, T_record={T_SIM}, T_burnin={T_BURNIN}, T_total={T_TOTAL}")


def run_simulation(seed):
    """Run one replication. Returns diagnostics dict + optional extra data."""
    ...
    for t_abs in range(1, T_TOTAL):
        ...
        # Permanent innovations with heterogeneity and jumps
        ...
        # Transitory innovations: t-distributed, variance-normalized
        # (no ARCH — this is the key simplification vs v3.6+)
        t_raw = sp_stats.t.rvs(df=t_df, size=N_FULL, random_state=rng)
        nu = sn_het * t_var_factor * t_raw
        c_state = phi_v * c_state + nu

        # Rank-dependent mean reversion toward global mean
        current_mean = np.mean(tau)
        kappa_r = kappa_base * (cr / N_FULL) ** alpha_kappa
        tau += eta - kappa_r * (tau - current_mean)

        # Exit/entry — ONLY during recording period (not burn-in)
    ...
    diag['zipf_slope'] = np.polyfit(
        np.log(np.arange(1, N_FULL+1)[mask]), np.log(ss[mask]), 1)[0]
...
for k in [2, 4, 8, 13]:
    ...
    print(f"  VR({k:2d}): emp={vr_emp[k]:.4f}  sim={s['mean']:.4f} "
          f"[{s['lo']:.4f}, {s['hi']:.4f}]  err={err:.1f}% [{ok}]")
...
for lag in [1, 2]:
    ...
    print(f"  ACF({lag}): emp={acf_emp[lag]:.4f}  sim={s['mean']:.4f} "
          f"[{s['lo']:.4f}, {s['hi']:.4f}]  err={err:.4f} [{ok}]")
...
for lag in [1, 4, 13]:
    ...
    ok = "Y" if err < 0.08 else "N"
    print(f"  RACF({lag:2d}): emp={racf_emp[lag]:.4f}  sim={s['mean']:.4f} "
          f"[{s['lo']:.4f}, {s['hi']:.4f}]  err={err:.4f} [{ok}]")
...
for k in [1, 4, 13, 26, 52]:
    if f'pers{k}' in mc_stats:
        ...
        print(f"  k={k:2d}: emp={pers_emp[k]}  sim={s['mean']:.1f} "
              f"[{s['lo']:.0f}, {s['hi']:.0f}]  diff={d:+.1f} [{ok}]")
...
for k in [1, 4, 13, 26, 52]:
    if f'xr2_{k}' in mc_stats:
        ...
        ok = "Y" if err < 0.08 else "N"
        print(f"  R²({k:2d}): emp={xr2_emp[k]:.4f}  sim={s['mean']:.4f} "
              f"[{s['lo']:.4f}, {s['hi']:.4f}]  err={err:.4f} [{ok}]")
...
for key, label, emp_val in [
    ...
    if emp_val is not None:
        print(f"  {label}: emp={emp_val:.2f}  sim={s['mean']:.2f} "
              f"[{s['lo']:.2f}, {s['hi']:.2f}]")
    else:
...
print(f"  Cross-sec var: start={s_start['mean']:.2f}  "
      f"end={s_end['mean']:.2f}  (emp={xsec_var_full:.2f})")
...
print(f"\n{'='*70}")
print(f"SUMMARY — MINIMAL MODEL v1 ({N_REP} replications)")
print(f"{'='*70}")
...
if rep_extra is not None:
    ...
    fig.suptitle(f'Minimal Rank Diffusion Model v1 | {n_pass}/{len(tests)} | '
                 f'{N_REP} MC reps | {elapsed:.0f}s',
    ...
    sa=[np.nanmedian([sim_ch[i].dropna().autocorr(l) for i in range(min(500,N_BP))
                      if len(sim_ch[i].dropna())>l+5]) for l in lags]
    ...
    sim_racf_rep = [np.nanmedian([sim_rk_df_plot[i].dropna().autocorr(l)
                     for i in range(min(1000,N_BP))
                     if len(sim_rk_df_plot[i].dropna())>l+5]) for l in rl]
    ...
    ax.set_xscale('log'); ax.set_title('Band Variance'); ax.legend(fontsize=8)
    ...
    ax.set_xscale('log'); ax.set_title('Band VR(4)'); ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title('Pers(13) MC dist'); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ...
    ax.axvline(np.mean(vals), color='red', lw=2, label=f'Mean={np.mean(vals):.1f}')
    ax.set_title('Kurtosis MC dist'); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ...
    ax.set_title('RACF(1) MC dist'); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    plt.savefig('/Users/hindman/Documents/github/rank-diffusion/llm_fitting/minimal_v1_diagnostics.png',
                dpi=130, bbox_inches='tight')
    print("\nSaved minimal_v1_diagnostics.png")
    plt.close()

# Final summary
print(f"\n{'='*70}")
print(f"MINIMAL MODEL v1 COMPLETE")
print(f"{'='*70}")
print(f"  Calibration: {n_pass}/{len(tests)} diagnostics pass")
print(f"  Parameters: σ_obs={sigma_obs:.4f}  σ_het={sigma_het:.4f}  t_df={t_df:.2f}")
print(f"              κ_base={kappa_base:.6f}  α_κ={alpha_kappa}")
print(f"              jump_p={jump_prob:.4f}  jump_s={jump_scale:.2f}")
print(f"              T_burnin={T_BURNIN}")
print(f"  Elapsed: {time.time()-t_start:.0f}s")
print(f"\n  Complexity comparison vs v3.9:")
print(f"    Minimal: 10 ingredients, ~600 lines")
print(f"    v3.9:    14 ingredients, ~1712 lines")
print(f"    Removed: ARCH(1), rank-dep t_df, 2-pass kurtosis cal, κ stab factor")
print("Done.")
